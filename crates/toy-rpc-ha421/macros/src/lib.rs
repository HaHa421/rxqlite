...
//!
//! This is exported in `toy_rpc_ha421` as the `toy_rpc_ha421::macros` module.
...
/// `toy_rpc_ha421::util::RegisterService` trait, which returns the handler hashmap
...
/// This macro implements the `toy_rpc_ha421::util::RegisterService` trait to allow
...
/// Implements `toy_rpc_ha421::pubsub::Topic` trait for a type. The type will also be
...
pub fn derive_topic(item: proc_macro::TokenStream) -> proc_macro::TokenStream {
    ...
    let output = quote::quote! {
        impl toy_rpc_ha421::pubsub::Topic for #ident {
            type Item = #item;
