...
fn generate_transformed_trait_item(ident: &syn::Ident) -> syn::TraitItemMethod {
    ...
    syn::parse_quote!(
        fn #handler_ident(
            ...
            deserializer: Box<dyn toy_rpc_ha421::erased_serde::Deserializer<'static> + Send>
        ) -> toy_rpc_ha421::service::HandlerResultFut;
    )
}
    ...
    for (handler_item, orig_item) in items {
        if let syn::FnArg::Typed(pt) = orig_item.sig.inputs.last().unwrap() {
            ...
            let f: syn::ImplItemMethod = syn::parse_quote!(
                fn #handler_ident(
                    self: std::sync::Arc<Self>,
                    mut deserializer: Box<dyn toy_rpc_ha421::erased_serde::Deserializer<'static> + Send>
                ) -> toy_rpc_ha421::service::HandlerResultFut
                {
...
pub(crate) fn impl_local_registry_for_trait(
    ...
    let ret = quote::quote! {
        pub trait #registry_ident {
            fn handlers() -> std::collections::HashMap<&'static str, toy_rpc_ha421::service::AsyncHandler<Self>>;
        ...
        {
            fn handlers() -> std::collections::HashMap<&'static str, toy_rpc_ha421::service::AsyncHandler<Self>> {
                let mut map = std::collections::HashMap::<&'static str, toy_rpc_ha421::service::AsyncHandler<Self>>::new();
...
pub(crate) fn impl_register_service_for_trait_impl(
    ...
    let ret = quote::quote! {
        impl toy_rpc_ha421::util::RegisterService for #type_path {
            fn handlers() -> std::collections::HashMap<&'static str, toy_rpc_ha421::service::AsyncHandler<Self>> {
...
pub(crate) fn generate_service_client_for_trait(
    ...
    let client_struct: syn::Item = syn::parse_quote!(
        pub struct #client_ident<'c> {
            client: &'c toy_rpc_ha421::client::Client,
            service_name: &'c str,
        }
    );
...
pub(crate) fn generate_client_stub_for_trait(
    ...
    let stub_impl: syn::ItemImpl = syn::parse_quote!(
        impl #stub_ident for toy_rpc_ha421::client::Client {
            fn #stub_fn<'c>(&'c self) -> #client_ident {
                #client_ident {
                    client: self,
...
pub fn generate_trait_impl_for_client(input: &syn::ItemTrait) -> syn::ItemImpl {
    ...
    let mut output: syn::ItemImpl = syn::parse_quote!(
        impl #service_ident for toy_rpc_ha421::client::Client {
