    ...
    quote::quote! {
        macro_rules! toy_rpc_handler {
            // handler when return type is a Result
             // handler when return type is either a toy_rpc_ha421 or anyhow Result
             ($self:ident, $method:ident, $de:ident, $req_ty:ty, Result<$ok_ty:ty>) => {
                Box::pin(
                    async move {
                        let req: $req_ty = toy_rpc_ha421::erased_serde::deserialize(&mut $de)
                            .map_err(|e| toy_rpc_ha421::error::Error::ParseError(Box::new(e)))?;
                        // let result: Result<$ok_ty, $err_ty> = $self.$method(req).await;
                        let outcome: $ok_ty = $self.$method(req).await?;
                        Ok(Box::new(outcome) as toy_rpc_ha421::service::Outcome)
                    }
                )
            };
            ...
            // handler when return type is either a toy_rpc_ha421::Result
            ($self:ident, $method:ident, $de:ident, $req_ty:ty, toy_rpc_ha421::Result<$ok_ty:ty>) => {
                toy_rpc_handler!($self, $method, $de, $req_ty, Result<$ok_ty>)
            };
...
pub(crate) fn macro_rules_client_stub() -> proc_macro2::TokenStream {
    quote::quote! {
        macro_rules! toy_rpc_client_stub {
            ...
            ($self:ident, $service_method:expr, $req_id:ident, toy_rpc_ha421::Result<$ok_ty:ty>) => {
                toy_rpc_client_stub!($self, $service_method, $req_id, Result<$ok_ty>)
            };
            ...
            ($self:ident, $func:ident, $service_method:expr, $req_ty:ty, Result<$ok_ty:ty>) => {
                pub fn $func<A>(&'c $self, args: A) -> toy_rpc_ha421::client::Call<$ok_ty>
                where
                    A: std::borrow::Borrow<$req_ty> + Send + Sync + toy_rpc_ha421::serde::Serialize + 'static,
                {
            ...
            ($self:ident, $func:ident, $service_method:expr, $req_ty:ty, toy_rpc_ha421::Result<$ok_ty:ty>) => {
                toy_rpc_client_stub!($self, $func, $service_method, $req_ty, Result<$ok_ty>);
            };
            ...
            ($self:ident, $func:ident, $service_method:expr, $req_ty:ty, $ret_ty:ty) => {
                pub fn $func<A>(&'c $self, args: A) -> toy_rpc_ha421::client::Call<$ret_ty>
                where
                    A: std::borrow::Borrow<$req_ty> + Send + Sync + toy_rpc_ha421::serde::Serialize + 'static,
                {
                    $self.client.call($service_method, args)
