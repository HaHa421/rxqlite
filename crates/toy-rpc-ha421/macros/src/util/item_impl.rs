...
pub(crate) fn transform_impl_item(f: &mut syn::ImplItemMethod) {
    ...
    if let syn::FnArg::Typed(pt) = f.sig.inputs.last().unwrap() {
        ...
        f.sig.inputs = syn::parse_quote!(
            self: std::sync::Arc<Self>, mut deserializer: Box<dyn toy_rpc_ha421::erased_serde::Deserializer<'static> + Send>
        );
...
/// Generate implementation of the `toy_rpc_ha421::util::RegisterService` trait.
///
    ...
    let ret = quote::quote! {
        impl toy_rpc_ha421::util::RegisterService for #type_path {
            fn handlers() -> std::collections::HashMap<&'static str, toy_rpc_ha421::service::AsyncHandler<Self>> {
                let mut map = std::collections::HashMap::<&'static str, toy_rpc_ha421::service::AsyncHandler<#type_path>>::new();
